...
(
    ...
    `email`      VARCHAR(245) UNIQUE NULL COMMENT 'Adres email',
...
-- -----------------------------------------------------
-- Table `scrumlab`.`meal_name`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `scrumlab`.`meal_name`
(
    `id`            INT         NOT NULL AUTO_INCREMENT,
    `name`     VARCHAR(45) NULL COMMENT 'Nazwa posiłku.',
    `display_order` INT         NULL COMMENT 'Kolejność wyświetlania.',
    PRIMARY KEY (`id`)
)
    ENGINE = InnoDB
    COMMENT = 'Tabela zawierająca nazwy posiłków.';
...
(
    ...
    `meal_name_id`  INT          NOT NULL COMMENT 'Klucz obcy tabeli posiłków',
    ...
    INDEX `fk_recipe_plan_meal_name_idx` (`meal_name_id` ASC),
    ...
    CONSTRAINT `fk_recipe_plan_plan1`
        FOREIGN KEY (`plan_id`)
            ...
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
     CONSTRAINT `fk_recipe_plan_meal_name`
            FOREIGN KEY (`meal_name_id`)
            REFERENCES `scrumlab`.`meal_name` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB
